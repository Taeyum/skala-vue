...
│   └── windField.js                    바람 벡터 분해·IDW 보간·격자 캐시 (순수 모듈)
├── views/                              페이지 단위 컴포넌트
│   ├── WeatherHomeView.vue             메인 대시보드 (히어로 + 타일 그리드)
│   ├── WeatherMapView.vue              전국 지도 (기온 채색 + 바람)
...
│   │   ├── KoreaMapStage.vue           지도 레이어 조립 (경계·입자·화살표·마커)
│   │   ├── KoreaWindCanvas.vue         바람 입자 애니메이션 (Canvas)
│   │   └── MapLegend.vue               기온 그라디언트 범례 + 바람 세기
│   └── practices/
│       ├── basic/                      Ch3 실습 컴포넌트
...
| 데이터 | 출처 | 라이선스 |
|---|---|---|
| 시도 행정구역 경계 | [southkorea/southkorea-maps](https://github.com/southkorea/southkorea-maps) — `kostat/2013/json/skorea_provinces_geo_simple.json` | 통계청(KOSTAT) 제공, 자유롭게 공유·변형 가능 |

원본 143KB를 좌표 소수점 5자리로 반올림해 78KB로 줄여 `src/assets/`에 두었습니다.
2013년 기준이라 강원도·전라북도가 특별자치도 개편 이전 이름이지만 경계는 같고, 화면에 시도 이름을 쓰지 않아 영향이 없습니다.
...
<details>
<summary><b>2026-08-22 — 전국 지도와 바람 흐름</b></summary>

<br>

한국 지도 위에 시도별 기온을 칠하고 바람이 흐르는 것을 보여주는 화면입니다.
지도 라이브러리(Leaflet, D3) 없이 GeoJSON을 직접 SVG path로 바꿔 그렸습니다.
예보 차트를 `polyline`으로 만들었던 것과 같은 방식입니다.

도시가 6개면 지도가 비어 보여서 광역시·도청 소재지 기준 17개로 늘렸습니다.
시도당 하나씩이라 각 시도에 그 지점의 기온을 그대로 칠합니다.

**단순 선형 변환 대신 메르카토르를 썼습니다.**

경도·위도를 그대로 x·y에 대응시키면 위도 36도에서 가로가 눌려 지도가 뚱뚱해집니다(폭/높이 1.16).
등장방형에 `cos(기준위도)`를 곱하면 비율은 맞지만, 기준 위도에서 멀어질수록 x 축척이 어긋납니다.

이게 문제가 되는 이유는 **바람 화살표** 때문이었습니다.
x와 y의 축척이 다르면 각도가 찌그러져서, 남쪽과 북쪽 화살표가 같은 방향을 가리켜도 화면에서는 다르게 보입니다.
메르카토르는 어느 지점에서나 x·y 축척이 같은 등각 투영이라 이 문제가 아예 생기지 않습니다.
결국 "위도별로 cos 보정을 연속적으로 적용한 것"이 메르카토르였습니다.

**전남 안의 광주는 `fill-rule="evenodd"` 하나로 해결됐습니다.**

경계 데이터를 열어보니 전남에 구멍(inner ring)이 두 개 있었습니다. 광주를 도려낸 자리입니다.
바깥 고리와 구멍을 한 `path`에 이어 붙이고 `evenodd`를 주면 구멍이 뚫리고, 그 자리를 광주 폴리곤이 채웁니다.

같은 성질이 육지 판정에도 그대로 쓰였습니다.
모든 고리에 광선투사를 하면 광주 안의 점은 `전남 바깥(1) + 전남 구멍(1) + 광주 바깥(1) = 3`, 홀수라서 육지로 나옵니다.
구멍을 따로 처리하는 코드를 쓰지 않았습니다.

**풍향은 각도가 아니라 성분으로 평균해야 했습니다.**

관측 지점이 17개뿐이라 그 사이는 역거리 가중(IDW)으로 채웁니다.
처음에는 각도를 그대로 평균하려 했는데, 359도와 1도의 평균이 180도가 되어 정반대 방향이 나옵니다.
x·y 성분으로 분해해 각각 평균한 뒤 다시 각도로 되돌리니 해결됐습니다.
Node로 이 경우를 따로 검산해서 중간값이 정확히 180도가 되는 것을 확인했습니다.

**`rotate(풍향 + 180)` 한 줄로 화살표 방향이 맞았습니다.**

OpenWeather의 `wind.deg`는 바람이 **불어오는** 방향이라 화살표는 반대를 가리켜야 합니다.
그런데 SVG 회전은 시계방향이고 나침반 방위각도 북 기준 시계방향이며, 지도는 위쪽이 북쪽입니다.
세 규칙이 같아서 북쪽을 향한 화살표 모양을 그려 두고 `rotate(deg + 180)`만 걸면 그대로 맞습니다.
좌표 변환 코드가 필요 없었습니다.

**입자는 격자를 거쳐 움직입니다.**

입자 900개가 매 프레임 17개 지점을 다시 계산하면 초당 92만 번이 됩니다.
41×44 격자에 바람장을 미리 계산해 두고(0.8ms) 입자는 네 꼭짓점을 섞어 쓰게 바꿨습니다.
측정해 보니 프레임당 계산이 0.21ms로 16.7ms 예산의 1.3%였습니다.

**꼬리는 색을 덮는 게 아니라 알파를 깎습니다.**

흔히 쓰는 방법은 반투명한 검은 사각형을 매 프레임 덮는 것인데,
이건 배경이 불투명한 캔버스에서만 통합니다. 지도 위에 얹은 투명 캔버스에 쓰면 아래 지도가 점점 검게 물듭니다.
`destination-out`으로 알파만 깎으니 캔버스는 투명하게 남고 꼬리만 옅어졌습니다.

다만 8비트 알파가 반올림 때문에 완전히 0이 되지 않았습니다.
실제로 재보니 알파 242에서 시작해 50프레임 만에 4까지 떨어진 뒤 400프레임이 지나도 4 그대로였습니다.
200프레임마다 한 번씩 완전히 지우도록 했습니다.

**바다는 마스크로 가립니다.**

입자가 바다로 나가지 않게 하려면 매 프레임 클리핑을 걸어야 하는데, 3,681정점짜리 경로를 60번씩 처리하는 셈이라 부담이 큽니다.
대신 육지가 뚫린 도형을 캔버스 **위에** 한 장 덮었습니다. 정적인 도형 하나라 프레임당 비용이 0입니다.

**마커는 점과 칩을 분리했습니다.**

세종과 대전이 화면에서 18px밖에 안 떨어져 칩이 서로 덮였습니다.
점은 실제 좌표에 두고 칩만 비켜 놓은 뒤 얇은 선으로 이었습니다. 지도에서 흔히 쓰는 방식입니다.
칩에는 기온만 넣고 도시 이름은 고르거나 마우스를 올릴 때만 펼치게 했더니 17개가 겹치지 않고 들어갔습니다.
...
지도 화면을 만들면서 홈의 시간여행 로직(`forecastMap` / `timeIndex` / `projectCity`)을 그대로 복사했습니다.
아래 화씨 변환 중복과 같은 이유로, Composable로 묶는 것이 해결책이지만 학습 범위 밖이라 그대로 두었습니다.
