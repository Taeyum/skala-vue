...
│   └── videoStore.js                   Pexels 배경 영상
├── utils/
│   ├── weatherTheme.js                 그라디언트 / 이미지 검색어 / 기상 코드 한글 매핑
│   ├── lifeIndex.js                    생활 지수 규칙 (우산·옷차림·외출·빨래·세차·환기)
│   ├── localTime.js                    현지 시각 / 낮·밤 판정 / 일출·일몰 궤적
│   └── revealDirective.js              v-reveal 스크롤 진입 디렉티브
├── views/                              페이지 단위 컴포넌트
...
│   │   ├── LifeBriefing.vue            생활 브리핑 (상세 패널 전체 / 홈 히어로)
│   │   ├── TimeTravelBar.vue           시간여행 타임라인 (5일 예보 슬라이더)
│   │   ├── RollingNumber.vue           숫자 값 트윈 (rAF, 이전 값에서 굴러가는 전환)
│   │   └── CrossfadeBackground.vue     배경 크로스페이드 레이어 (그라디언트·사진)
│   └── practices/
│       ├── basic/                      Ch3 실습 컴포넌트
...
| Pexels — Video Search | 상세 페이지 배경 영상 (재생 준비 후 이미지 위에 페이드인) |
...
<details>
<summary><b>2026-08-21 — 상세 페이지 배경을 영상으로</b></summary>

<br>

Pexels Photo API로 받던 정지 이미지를 Video API 영상으로 바꿨습니다.
`photoStore`를 고치지 않고 `videoStore`를 새로 만들었습니다. 과제 6 제출물을 보존하고, 영상 조회가 실패하면 기존 이미지로 내려가는 폴백을 남기기 위해서입니다.

**화질은 폭 1280px 이하 중 가장 큰 mp4를 고릅니다.**
배경용이라 4K까지는 필요 없고, 응답의 `video_files`에 화질별 파일이 전부 들어 있어서 고르기만 하면 됐습니다.

**후보 10개 중 가장 긴 영상을 씁니다.**
처음엔 첫 번째 결과를 그대로 썼는데 흐림 날씨에서 12초짜리 핸드헬드 영상이 걸려 화면이 흔들렸습니다.
짧은 클립은 손으로 찍은 게 많고 `loop` 재시작 지점도 자주 돌아와서, `duration` 기준으로 정렬해 가장 긴 것을 고르니 타임랩스 계열이 잡혔습니다.
`per_page`를 10으로 늘려도 API 호출은 1번이고 받는 건 30KB 정도의 메타데이터뿐이라 부담이 없었습니다.

**도시를 바꿀 때 사진이 한 번 끼어드는 문제가 있었습니다.**
이전 영상을 지우고 새 영상을 기다리는 사이에 배경 이미지가 드러나는 것이었습니다.
영상을 배열(`videoLayers`)로 들고 있다가 새 영상이 `canplay`되면 이전 영상 위로 페이드인하고, 전환이 끝나면(`transitionend`) 아래 레이어를 지우도록 바꿨습니다.
같은 날씨로 이동하면 URL이 같아 끊지 않고 이어 재생합니다.

</details>

<details>
<summary><b>2026-08-21 — 디자인 토큰 도입</b></summary>

<br>

화면마다 만든 시점이 달라 모서리 둥글기가 8·10·12·14·16·18·20·24px로 흩어져 있었고, 회색도 네 가지가 섞여 있었습니다.
즐겨찾기·소개·실습·404는 불투명 흰 카드인데 홈과 헤더는 반투명 유리라 두 가지 언어가 공존하는 것이 톤 차이의 근본 원인이었습니다.

`base.css`의 `:root`에 색·여백·radius·그림자·폰트 크기를 CSS 변수로 정의하고, 각 화면의 `<style scoped>`에서 `var()`로 참조하도록 바꿨습니다.
radius는 `sm(8) / md(12) / lg(20) / pill` 네 단계, 그림자는 세 단계, 여백은 4px 배수로 제한했습니다.
흰 카드는 전부 유리 카드로 바꿔 홈과 같은 질감이 되게 했습니다.

과제 1·2 화면(`/mockup`)과 실습 컴포넌트 내부는 제출물이라 건드리지 않았습니다.
온도 숫자처럼 콘텐츠 성격이 강한 치수는 토큰으로 묶지 않고 그대로 두었습니다.

</details>

<details>
<summary><b>2026-08-21 — 생활 브리핑</b></summary>

<br>

습도 62%, 강수확률 40% 같은 숫자를 "빨래 널어도 되나", "우산 챙겨야 하나"라는 답으로 바꾸는 기능입니다.
이미 받아 둔 현재 날씨·3시간 예보·대기질만으로 계산하므로 API 호출이 늘지 않습니다.

| 지수 | 입력 | 규칙 |
|---|---|---|
| ☂️ 우산 | 앞 12시간 강수확률 | 60% 이상 필수 · 30% 이상 챙기면 좋음 |
| 🧥 옷차림 | 체감온도, 풍속 | 7단계 구간 + 7m/s 이상이면 바람막이 |
| 🌳 외출 | 대기질, 체감, 강수, 바람 | 감점 사유를 모아 가장 큰 것을 설명으로 |
| 🧺 빨래 | 습도, 풍속, 강수 | 건조 점수 = (100−습도)×0.6 + min(풍속,5)×8 |
| 🚗 세차 | 앞 36시간 강수확률 | 40% 이상 구간이 있으면 미루기 |
| 🪟 환기 | 대기질, 강수 | AQI 4 이상 자제 · 3이면 짧게 |

여기에 오늘 남은 3시간 구간을 채점해 "오늘 외출은 15시 ~ 21시" 같은 최적 시간대를 한 줄로 붙였습니다.

**판단은 `utils/lifeIndex.js`에, 컴포넌트는 그리기만 합니다.**

과제 3에서 "계산은 부모가 하고 자식은 그리기만 한다"고 정리했던 원칙을 그대로 가져왔습니다.
여섯 지수가 전부 `{ key, icon, title, level, label, desc }` 같은 모양을 돌려주기 때문에 `LifeBriefing.vue`는 `v-for` 하나로 끝납니다.
규칙을 고치고 싶으면 컴포넌트를 열 필요가 없고, Node에서 가짜 데이터로 바로 확인할 수 있었습니다.

**한 컴포넌트를 두 화면에서 다르게 씁니다.**

상세 패널에서는 지수 6개와 최적 시간대를 전부 보여주고, 홈 히어로에서는 `mode="compact"`로 우산·옷차림·외출 세 개만 칩으로 띄웁니다.
홈에 넣을 때 `WeatherHero`에 `<slot name="extra">`를 뚫었습니다.
slot 내용은 부모 스코프에서 컴파일되므로 `WeatherHero`는 브리핑이 뭔지 몰라도 되고, `WeatherHomeView`가 직접 데이터를 넘깁니다.

**홈에서 예보를 한 번 더 부르게 됐습니다.**

지금까지 홈은 현재 날씨만 썼는데 우산 지수에 예보가 필요해서, 선택 도시가 바뀔 때 `forecastStore.fetchForecast`를 호출합니다.
스토어가 도시별로 캐싱하고 있어서 상세로 들어가면 같은 데이터를 재사용하고, 한 도시당 1회를 넘지 않습니다.

**빠진 데이터는 판정하지 않고 `--`로 둡니다.**

처음에는 습도가 없으면 60%로 가정하는 식으로 기본값을 넣었는데, 가짜 데이터로 돌려보니 정보가 없는 도시에 "빨래 나쁨"이 떴습니다.
없는 값으로 만든 판정은 틀린 판정보다 나쁘다고 생각해서, 입력이 없으면 `--`와 "정보 없음"을 돌려주도록 바꿨습니다.
과제 1에서 `??`로 0도를 살렸던 것과 같은 맥락이었습니다.

</details>

<details>
<summary><b>2026-08-21 — 현지 시각과 낮·밤 모드</b></summary>

<br>

상세 페이지의 날짜·시각이 내 PC 시각이라, 도시 추가로 Tokyo나 London을 넣으면 틀린 시각이 떴습니다.
OpenWeather 응답에 `timezone`(UTC 오프셋)과 `sys.sunrise` / `sys.sunset`이 이미 들어 있어서 API 호출 없이 해결할 수 있었습니다.

**오프셋을 더한 뒤 UTC 메서드로 읽었습니다.**

`Date`는 항상 브라우저 타임존으로 동작해서 다른 도시의 시각을 직접 만들 수가 없습니다.
`new Date(now + timezone * 1000)`으로 오프셋만큼 밀어둔 다음 `getUTCHours()`로 읽으면 그 도시의 현지 값이 나옵니다.
`Intl.DateTimeFormat`에 타임존 이름을 넘기는 방법도 있지만 API가 주는 건 이름이 아니라 오프셋이라 이 쪽이 맞았습니다.

**낮·밤 판정은 타임존과 무관하게 했습니다.**

`sunrise`·`sunset`이 UTC 타임스탬프라 지금 시각과 바로 비교하면 됩니다.
표시용 문자열만 오프셋을 쓰고, 판정 로직에는 타임존이 들어가지 않습니다.

**"지금"을 ref로 두고 1분마다 갱신합니다.**

현지 시각과 "일몰까지 2시간 13분"이 움직여야 해서 `now` ref를 `setInterval`로 갱신하고, 나머지는 전부 `computed`로 파생시켰습니다.
덕분에 일몰 시각을 지나는 순간 낮·밤 판정이 뒤집히고, 그라디언트·애니메이션·배경 영상 검색어가 한 번에 야간으로 바뀝니다.
`onUnmounted`에서 타이머를 정리하지 않으면 페이지를 떠나도 계속 도는데, 3장 라이프사이클 실습에서 봤던 그 상황이었습니다.

**밤이면 바뀌는 것**

| 대상 | 낮 | 밤 |
|---|---|---|
| 홈 그라디언트 | 날씨별 밝은 팔레트 | 같은 날씨의 야간 팔레트 (`WEATHER_GRADIENT_NIGHT`) |
| 맑음 애니메이션 | 해 (pulse + 회전 광선) | 초승달 + 별 40개 깜빡임 |
| Pexels 검색어 | `clear blue sky` | `starry night sky` (캐시 키가 달라 따로 저장) |
| 타일 | — | 도시명 옆 🌙 |

초승달은 원 위에 같은 배경색의 `inset box-shadow`를 얹어 한쪽을 가리는 방식으로 그렸습니다.

**히어로 스탯을 4칸에서 6칸으로 늘렸습니다.**

일출·일몰을 넣으면서 2×2 그리드가 3×2가 됐습니다.
상세 페이지에는 일출→일몰 사이 어디쯤인지 보여주는 작은 궤적 바를 날짜 아래 두었습니다.

</details>

<details>
<summary><b>2026-08-21 — 시간여행 타임라인</b></summary>

<br>

홈 히어로 아래 슬라이더를 끌면 대시보드 전체가 그 시각의 예보로 바뀝니다.
히어로의 온도·상태·아이콘, 배경 그라디언트, 비·눈 애니메이션, 타일 여섯 개, 생활 브리핑이 전부 같이 움직이고, 일몰 뒤 구간으로 가면 화면이 밤이 됩니다.

**ref 하나가 전부를 바꿉니다.**

`timeIndex` 하나만 두고 나머지는 `computed`로 파생시켰습니다.
`targetSlot`(선택 구간) → `projectCity`(도시에 예보 값을 덮어쓴 객체) → `shownCity`·`sortedList`·`backgroundStyle`·`heroLabel`·`selectedIsNight`로 이어지는 체인입니다.
슬라이더가 `emit('update:modelValue')`로 숫자 하나를 올리면 반응형 시스템이 나머지를 알아서 다시 그립니다.
2장에서 `computed`가 "의존하는 값이 바뀔 때만 재계산된다"고 배운 것을 가장 크게 써먹은 자리였습니다.

**컴포넌트는 지금인지 예보인지 모릅니다.**

`WeatherHero`와 `WeatherTile`은 `city` 객체를 받아 그리기만 합니다.
시간여행을 넣으면서 이 컴포넌트들을 고치지 않기 위해, 현재 도시 객체에 선택 구간의 `temp`·`status`·`icon`·`main` 등을 덮어쓴 새 객체를 만들어 넘겼습니다.
히어로 라벨만 "현재 선택한 지역" 대신 "내일 오후 3시 예보"가 되도록 prop을 하나 열었습니다.

**정렬도 그 시각 기준입니다.**

`sortedList`가 현재 온도로 정렬하고 있어서, 시간을 옮겨도 타일 순서가 안 바뀌었습니다.
정렬 전에 `projectCity`를 먼저 적용하니 "내일 오후엔 제주가 가장 덥다"처럼 순위가 바뀌고, 과제 7에서 만든 `TransitionGroup`의 `move` 덕분에 타일이 미끄러지며 자리를 바꿉니다.
시간여행을 만들면서 가장 만족스러운 장면이었습니다.

**전 도시 예보를 미리 받습니다.**

타일까지 바꾸려면 선택 도시만이 아니라 모든 도시의 예보가 필요했습니다.
진입 시 `Promise.all`로 한 번에 받고, 도시가 추가되면 그 도시만 더 받습니다.
`forecastStore`가 앞 12개만 잘라 두던 것을 40개 전부 저장하도록 바꾸고, 차트는 화면에서 `slice(0, 12)`로 잘라 쓰도록 했습니다.

**예보 시각의 낮·밤은 일출·일몰을 평행이동해서 판정했습니다.**

API가 주는 일출·일몰은 오늘 것뿐인데 예보는 5일 뒤까지 갑니다.
하루 단위로 밀어서 비교하면 며칠 뒤라도 몇 분 오차로 맞습니다.

</details>

<details>
<summary><b>2026-08-22 — 모션 디자인 토큰과 prefers-reduced-motion</b></summary>

<br>

기능이 쌓이면서 화면마다 전환 속도와 곡선이 제각각이 됐습니다.
duration이 0.2 / 0.25 / 0.3 / 0.45 / 0.5 / 0.8 / 1s 일곱 종류로 흩어져 있었고,
easing은 대부분 지정을 생략해 기본 `ease`였습니다. `ease-out`은 프로젝트 전체에서 한 번도 쓰이지 않았습니다.

**radius·그림자처럼 모션도 토큰으로 묶었습니다.**

디자인 토큰을 도입할 때 색·여백·radius는 정리했는데 모션만 빠져 있던 게 원인이었습니다.
`base.css`에 duration 4단계(`--dur-1`~`--dur-4`: 눌림 / 표준 / 이동·등장 / 앰비언트)와
easing 3종(`--ease-out` / `--ease-in-out` / `--ease-emphasized`)을 정의하고 전 화면이 이것만 쓰게 했습니다.

기본 `ease`는 가속·감속이 대칭에 가까워 기계적으로 느껴집니다.
iOS 모션의 감각은 "빠르게 출발해서 길게 안착"이라, 기본값을 `--ease-out`(cubic-bezier(0.22, 1, 0.36, 1))로 잡았습니다.

**reduced-motion에서 duration을 0이 아니라 0.01s로 줄였습니다.**

처음에는 `* { transition: none !important }`로 전부 꺼버릴 생각이었는데,
상세 페이지의 배경 영상 레이어가 `transitionend` 이벤트로 아래 레이어를 지운다는 게 걸렸습니다.
transition을 없애면 이 이벤트 자체가 발화하지 않아서, 도시를 옮길 때마다 영상 레이어가 무한히 쌓입니다.
0.01s면 사람 눈에는 즉시 전환이지만 이벤트는 정상적으로 발화합니다.

무한 반복 keyframes(비·눈·별)는 duration 토큰의 영향을 받지 않아 따로 처리했습니다.
0.01s로 줄이면 초당 수십 번 깜빡이는 스트로브가 되기 때문에, 떨어지는 입자는 숨기고
해·달·구름은 `animation: none`으로 정지 상태로 남겨 분위기만 유지했습니다.

</details>

<details>
<summary><b>2026-08-22 — 숫자 롤링 전환</b></summary>

<br>

온도·습도·풍속 숫자가 상태가 바뀔 때마다 즉시 갈아끼워지고 있었습니다.
시간여행 슬라이더를 밀면 전국 온도가 함께 "흘러가는" 것이 목표였습니다.

**자릿수 슬롯 롤링 대신 값 트윈을 골랐습니다.**

iOS 시계처럼 0~9 숫자 기둥이 도는 방식도 고민했는데,
음수 온도·자릿수 변화(9→10)·소수점(풍속 3.6)에서 구현이 급격히 복잡해지고,
각 자리가 따로 돌면 "값이 흐른다"는 연속성이 오히려 약해집니다.
`requestAnimationFrame`으로 23 → 24 → 25 → … → 37처럼 중간값을 실제로 경유하는 값 트윈이
목표에 정확히 맞았고, `RollingNumber.vue` 컴포넌트 하나(60줄)로 끝났습니다.

**트윈 도중 새 값이 오면, 마지막으로 그린 값에서 방향만 바꿉니다.**

슬라이더를 빠르게 긁으면 트윈이 끝나기 전에 다음 값이 도착합니다.
이전 목표값에서 다시 시작하면 숫자가 점프하기 때문에,
"현재 화면에 그려진 실수값"을 항상 들고 있다가 거기서 새 목표로 이어가게 했습니다.
37에서 23으로 내려가다 중간에 다시 37을 누르면 27쯤에서 그대로 방향을 돌립니다.

**`--`는 트윈하지 않고 스냅합니다.**

관측 불가 도시는 값이 없는데, 값이 생길 때 0부터 굴러 올라오면 존재하지 않던 연속성을 지어내는 셈입니다.
없던 값 → 있는 값, 있는 값 → `--`는 즉시 전환하게 했습니다.
과제 1에서 `??`로 0도를 지켰던 것과 같은 맥락이라고 생각했습니다.

숫자 폭이 흔들리면 리플로우가 생기므로 컴포넌트에 `font-variant-numeric: tabular-nums`를 내장했습니다.

</details>

<details>
<summary><b>2026-08-22 — 배경 크로스페이드</b></summary>

<br>

**기존 배경 전환은 사실 동작한 적이 없었습니다.**

홈 배경에 `transition: background 0.8s`를 걸어뒀는데, `linear-gradient`는 CSS가 보간할 수 없는 값이라
실제로는 그냥 스냅되고 있었습니다. 상세 페이지의 `background-image` 전환도 마찬가지였습니다.
transition을 걸어두고 부드럽다고 믿고 있었던 죽은 코드였습니다.

**레이어를 쌓아서 opacity로 교차시켰습니다.**

배경이 바뀌면 새 레이어를 위에 얹고 opacity 0 → 1로 페이드인, 전환이 끝나면(`transitionend`) 아래 레이어를 제거합니다.
상세 페이지 배경 영상에서 이미 검증한 패턴이라, 홈과 상세가 같이 쓰도록 `CrossfadeBackground.vue`로 분리했습니다.
이벤트가 유실돼도 레이어가 무한히 쌓이지 않게 push 시점에 최대 3장 상한을 뒀습니다.

**`background`가 아니라 `background-image`로 바인딩했습니다.**

처음에 인라인 `background` 단축 속성으로 넣었더니 CSS의 `background-size: cover`가 무시됐습니다.
단축 속성은 지정하지 않은 하위 속성까지 초기값으로 리셋하기 때문입니다.
`background-image` 롱핸드로 바꾸니 해결됐고, 그라디언트도 CSS에서는 이미지라서 같은 속성으로 처리됩니다.

사진 배경은 `new Image()`로 로딩을 끝낸 뒤에 레이어를 올립니다.
로딩 중인 이미지가 페이드인하면 빈 레이어가 번쩍이기 때문입니다.

</details>

<details>
<summary><b>2026-08-22 — 화면 전환: 히어로 스왑, 라우트 방향성, 스켈레톤, 스크롤 진입</b></summary>

<br>

**히어로는 `:key` 하나로 두 가지 전환을 구분합니다.**

도시를 바꾸면 이전 도시가 위로 물러나고 새 도시가 아래에서 올라오는 스왑이 필요한데,
시간여행으로 예보 시각을 옮길 때는 히어로가 통째로 갈리면 안 되고 숫자만 흘러야 합니다.
`<Transition mode="out-in">` 안의 `WeatherHero`에 `:key="selectedId"`를 주니 이게 저절로 구분됐습니다.
시간여행은 `selectedId`가 그대로라 재마운트가 없고, 도시 클릭만 key가 바뀌어 스왑이 발동합니다.

**라우트 전환에 방향을 넣었습니다.**

각 라우트에 `meta.depth`(nav 좌→우 순서)를 달고, 이동할 때 이전 depth와 비교해
깊어지면 오른쪽에서, 얕아지면 왼쪽에서 미끄러져 들어오게 했습니다.
전환 CSS는 App.vue에 두는데 `scoped`로는 안 됩니다 —
Transition 클래스가 App이 아니라 각 뷰의 루트 요소에 붙기 때문입니다. 전역 `<style>` 블록으로 분리했습니다.
`scrollBehavior`도 이때 추가해서, 새 페이지는 맨 위로, 뒤로 가기는 보던 위치로 돌아갑니다.

**로딩 텍스트를 실물 크기 스켈레톤으로 바꿨습니다.**

"불러오는 중입니다..." 한 줄 대신 히어로와 타일 6개 자리를 실측 높이 그대로 잡아두고 shimmer를 돌립니다.
높이가 실물과 같아야 데이터가 도착해도 레이아웃이 점프하지 않습니다.
캐싱된 재방문에는 스켈레톤이 아예 안 뜨는데, 이건 의도한 것입니다 — 기다릴 일이 없는데 기다리는 척을 할 이유가 없습니다.
shimmer는 `transform: translateX`만 써서 layout 비용이 없습니다.

**스크롤 진입은 Composable 대신 커스텀 디렉티브로 만들었습니다.**

IntersectionObserver 로직을 화면마다 복사하기는 싫고, `useReveal()` 같은 Composable은 강의 범위 밖입니다.
커스텀 디렉티브는 Vue 기본 기능이라 `v-reveal` 속성 하나로 붙일 수 있었습니다.
reduced-motion 환경에서는 숨김 클래스를 아예 붙이지 않아서,
전환이 실패해도 콘텐츠가 가려진 채 남는 사고가 구조적으로 불가능합니다.

**숨겨진 탭에서는 rAF가 아예 멈춘다는 것도 이번에 알았습니다.**

브라우저 자동화로 트윈을 측정하는데 값이 전혀 안 움직여서 한참 헤맸습니다.
탭이 `visibilityState: hidden`이면 브라우저가 `requestAnimationFrame`을 실행하지 않고,
다시 보이는 순간 경과 시간 기준으로 최종값을 따라잡습니다.
트윈을 경과 시간 기반으로 만든 덕에 백그라운드 탭에서도 상태가 어긋나지 않는다는 걸 확인한 셈이 됐습니다.
